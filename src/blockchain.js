...
class Blockchain {
  ...
  submitStar(address, message, signature, star) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      const timeDiff = self.diffMinutes(currentTime, messageTime)

      if (timeDiff > 5) {
        reject('More than five minutes elapsed!')
      }
  ...
  getBlockByHash(hash) {
    let self = this
    return new Promise((resolve) => {
      const [block] = self.chain.filter((_block) => _block.hash === hash)
      resolve(block)
    })
  }
      ...
      self.chain.forEach((block, index) => {
        if (block.height > 0) {
          ...
          if (block.previousBlockHash !== previousBlock.hash) {
            const errorMessage = `Wrong previous hash ${block.previousBlockHash} in block ${index}`
